...
import { commercialStatusLabel, publicationStatusLabel, priorityLabel } from "@/lib/format";
import type { CommercialStatus, PublicationStatus, AnnouncementPriority } from "@/lib/format";
...
export function PriorityBadge({ priority }: { priority: AnnouncementPriority }) {
  const cls =
    priority === "urgente" ? "bg-destructive/10 text-destructive border-destructive/30"
    : priority === "importante" ? "bg-amber-100 text-amber-800 border-amber-200 dark:bg-amber-900/30 dark:text-amber-200"
    : "bg-accent/10 text-accent border-accent/20";
  return <Badge variant="outline" className={cls}>{priorityLabel[priority]}</Badge>;
}
